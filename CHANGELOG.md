...
## 2.0.0-rc.6 — выход из двух подряд NO-GO и операторский выбор

- После каждого NO-GO строится детерминированный разбор повторяемости по всей
  линии восстановления и попадает в контекст следующего ремонта; повторная форма
  ремонта больше не считается достаточной.
- При исчерпании остатка вместо ошибки появляется ограниченная диагностика с
  закрытым набором классов, обоснованием и вариантами; разбор привязан к точному
  состоянию линии — история ревью, принятый план и payload.
- Исчерпание становится штатным состоянием `awaiting-operator-decision` (код 3) с
  сохранением истории, accounting и frozen identity; `status` показывает состояние.
- Выбор оператора фиксируется неизменяемой записью и доводится до продолжения;
  переход, требующий отдельного маршрута, останавливает доставку вместо подмены
  ремонтом и не расходует слот. Автоматического продолжения без решения нет.
- Изменение формулировки критерия — отдельное операторское решение с сохранением
  прежней формулировки, автора и времени; применяется обычным маршрутом принятия.
- Маршрут технического восстановления в исчерпании не предлагается: существующее
  восстановление принимает только run до первого ревью. Класс `infrastructure`
  сообщает средовую причину и не даёт полномочий.
- Первые два независимых ревью автономны; каждое последующее требует отдельного
  ...
  наследуется без сброса.
- Устранены семь падений полного набора, убраны повторные запуски OpenSpec CLI из
  прогона, снят ложный блокер git-атрибутов в release-проверке и тестовый harness
  больше не забивает `/tmp` неудаляемыми basetemp.

Это подготовленный кандидат; CI точного release commit и публикация выполняются
отдельно. Подробности — в [release notes](docs/releases/2.0.0-rc.6.md).
