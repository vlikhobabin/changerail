...
завершенную story. `opsx-do` реализует, проверяет, синхронизирует specs и
архивирует card-owned changes, но оставляет карточку в `3.inprogress`.
Independent review проверяет именно этот полный payload. `opsx-pub` после
...
OPSX не навязывает каждому проекту одинаковую toolchain-matrix. Обязательный
...
Для длительных supervised запусков OPSX предоставляет tracked helper
`bin/opsx-delivery-runner`. Он запускает `$opsx-deliver <card>` через
repo-scoped `bin/codex`, закрывает stdin child-процесса и пишет
`.runtime/opsx/delivery-runs/<run-id>/status.json` с contract
`opsx.delivery-run.v1`. Supervisor наблюдает этот status record, а не `pgrep`
...
`<workspace>/.codex`, а не path OPSX source of truth. Если `--runtime-root` не
указан, status и logs пишутся под
`<workspace>/.runtime/opsx/delivery-runs/`. Перед background-run используйте
...
`bin/opsx-delivery-metrics` читает `.runtime/opsx/delivery-runs/*/status.json`
и `.runtime/opsx/reviews/*.history.json`, чтобы показать first-pass go rate,
