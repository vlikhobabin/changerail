...
пересматривает решение — например потому, что выбранный маршрут оказался
невыполним — он записывает новое решение со своей причиной, и именно последняя
запись ведёт продолжение; прежние остаются историей.
...
закрытие попытки, пересмотр плана или изменение критерия — runner не подменяет
его обычным ремонтом: он останавливается и называет нужный маршрут. Выданный при
этом слот не расходуется. Такой выбор не расходуется преемником: он продолжает
блокировать потомков, пока оператор не выполнит отдельный маршрут или не запишет
новое решение со своей причиной.
