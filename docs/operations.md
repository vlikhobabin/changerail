...
Амендмент сохраняет прежнюю формулировку как историю, не выдаёт слот, не
отменяет собранные proofs и не делает доставку успешной: после него требуется
обычное независимое ревью на текущий payload.

Продолжение после решения: слот даёт `review-allow` (+1), а выбранный вариант
определяет, как он расходуется. `review`/`re-review` повторяет независимое
ревью на неизменённом payload; `systemic-repair`/`continue-repair` запускает
ремонт с явно изменённым подходом; `revise-plan`/`extend-proof-model` требуют
закрыть попытку и принять отдельный новый план; `technical-recovery` уходит в
существующий маршрут восстановления. Автоматического продолжения без решения
оператора нет: разбор сам ничего не разрешает и не расходует.
