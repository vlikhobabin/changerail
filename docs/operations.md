...
## Разбор исчерпания и продолжение после двух NO-GO

Два подряд NO-GO — рабочая ситуация, а не тупик. Runner не просто
останавливается: он разбирает всю историю ревью и предъявляет класс ситуации с
вариантами перехода.

После **каждого** NO-GO строится детерминированный разбор повторяемости: какие
условия провалены повторно, какие закрыты, какие провалены впервые, и какие
findings ссылаются на уже названные пути. Разбор попадает в контекст следующего
ремонта, поэтому ремонт видит историю, а не только последний список findings.
При повторяющемся условии инструкция прямо требует изменить подход.

При исчерпании остатка появляется артефакт диагностики. Посмотреть его:

```sh
"$chrl_project/bin/chrl" --project "$chrl_project" rethink "$chrl_run"
```

Класс ситуации определяет, какие переходы предложены:

К любому разбору добавляется `technical-recovery`: право на это даёт не класс,
а собственный гейт технического восстановления, и класс, единственный переход
которого гейт отклонил, не должен оставлять оператора без выбора.

| Класс | Что означает | Переходы |
|---|---|---|
| `repeat_defect` | тот же дефект повторяется после ремонта | `systemic-repair`, `revise-plan` |
| `incomplete_work` | каждый раз находятся новые несделанные части | `continue-repair`, `revise-plan` |
| `plan_conflict` | принятый план и требования ревью несовместимы | `revise-plan`, `amend-criterion` |
| `evidence_model_gap` | требуемое доказательство невыразимо текущей моделью proofs | `extend-proof-model`, `amend-criterion`, `revise-plan` |
| `unreproducible` | наблюдение ревьюера не воспроизводится | `re-review`, `systemic-repair` |
| `infrastructure` | сбой среды, не продуктовый дефект | `systemic-repair`, `revise-plan`, `amend-criterion`, `close-attempt` |
| `unsatisfiable` | критерий невыполним в текущей формулировке | `amend-criterion`, `revise-plan` |

Автоматического маршрута нет ни у одного класса. Существующее техническое
восстановление сознательно принимает только исходный остановленный run до
первого ревью: линия, уже прошедшая ревью, не проходит его границу, поэтому
предлагать его здесь было бы пустым обещанием. Класс `infrastructure`
показывает, что причина, вероятно, средовая, и оставляет оператору маршруты,
которые действительно могут выполниться. Расширение технического
восстановления на линии после ревью — отдельное изменение инструмента.

Исчерпание — штатная остановка, а не сбой: run завершается кодом `3`, в его
`run.json` появляются `stop_state` = `awaiting-operator-decision` и
`awaiting_decision` с классом, обоснованием и доступными вариантами, а `chrl
status` показывает их вместе с остатком слотов. Код `2` по-прежнему означает
настоящий отказ. Повторный `resume` уже исчерпанного run не запускает работу и
не переписывает историю: он снова возвращает ту же точку решения, пока оператор
не зафиксирует выбор. Поэтому «остановилось с кодом 3» — это не поломка,
а приглашение выбрать маршрут.

Если в профиле задан route `[models.diagnosis]`, неопределённая ситуация
уточняется одной ограниченной сессией диагностики. Сессия не реализует, не
ревьюирует, не ремонтирует и не публикует; она пишет только артефакт диагноза.
Runner отвергает класс, который отрицает уже наблюдаемое: повторяющееся условие
нельзя объявить «неполнотой», а новое падение — повтором. Без этого route
работает только детерминированный разбор, как раньше.

Выбор фиксируется двухфазно и неизменяемо, как и `review-allow`: запись
связывается с точным разбором и состоянием линии, тот же выбор по той же причине
идемпотентен, а та же причина не может молча нести другой переход. Если оператор
пересматривает решение — например потому, что гейт технического восстановления
отклонил выбранный маршрут — он записывает новое решение со своей причиной, и
именно последняя запись ведёт продолжение; прежние остаются историей.

```sh
"$chrl_project/bin/chrl" --project "$chrl_project" rethink "$chrl_run" \
  --option systemic-repair --reason 'тот же подход дважды не сработал'
# проверьте digest и передайте его
"$chrl_project/bin/chrl" --project "$chrl_project" rethink "$chrl_run" \
  --option systemic-repair --reason 'тот же подход дважды не сработал' \
  --authorize "$chrl_choice_sha256"
```

Если диагностика показала, что критерий невыполним или доказательство
невыразимо, оператор может изменить формулировку критерия отдельной записью:

```sh
"$chrl_project/bin/chrl" --project "$chrl_project" rethink "$chrl_run" \
  --amend-criterion C1 \
  --before 'прежняя формулировка' --after 'новая формулировка' \
  --reason 'критерий в такой формулировке недостижим'
```

Амендмент сохраняет прежнюю формулировку как историю, связывает запись с
оператором и временем, не выдаёт слот, не отменяет собранные proofs и не делает
доставку успешной. Саму новую формулировку амендмент не применяет: её вносят в
карточку и принятый план обычным маршрутом принятия, и только после этого
доставка может продолжиться — обычное независимое ревью на текущий payload
остаётся обязательным.

Продолжение после решения идёт в таком порядке: `rethink` показывает разбор,
`rethink --option` фиксирует выбор, `review-allow` выдаёт +1, `resume` продолжает
работу. Записанное решение не остаётся в стороне: оно попадает в
recovery-контекст преемника и в контекст ремонта, поэтому рабочая сессия видит
выбранный вариант, его причину и требуемый подход, а не только список findings.
Расходуется слот так:

- `re-review` — payload не меняется: runner заново фиксирует наблюдаемый scope и
  детерминированный pre-review floor для тех же байтов и повторяет независимое
  ревью, не запуская ремонт. Если floor или scope не восстанавливаются, вместо
  этого идёт обычный ремонт: ревью не отправляют то, что гейт не пропускает.
- `systemic-repair`/`continue-repair` — обычный ремонт и независимое ревью;
  повторять прежнюю форму ремонта недостаточно, выбранный вариант задаёт подход.
- `revise-plan`/`extend-proof-model` — текущая попытка закрывается как
  неуспешная и отдельно принимается новый план; выбор фиксирует намерение,
  закрытие остаётся отдельным операторским действием.
- `technical-recovery` — уходит в существующий маршрут восстановления и не
  расходует продуктовый бюджет.

Автоматического продолжения без решения оператора нет: разбор сам ничего не
разрешает и не расходует. Если записанный выбор требует отдельного маршрута —
закрытие попытки, пересмотр плана, изменение критерия или техническое
восстановление — runner не подменяет его обычным ремонтом: он останавливается и
называет нужный маршрут. Выданный при этом слот не расходуется.
