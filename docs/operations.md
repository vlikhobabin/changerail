...
| `infrastructure` | сбой среды, не продуктовый дефект | `technical-recovery` |
| `unsatisfiable` | критерий невыполним в текущей формулировке | `amend-criterion`, `revise-plan` |

Класс `infrastructure` — единственный автоматический маршрут: он ведёт в
существующее техническое восстановление и не расходует продуктовый бюджет.
Остальные классы требуют явного решения оператора.
...
Выбор фиксируется двухфазно, как и `review-allow`:
