...
проекты как прямые потребители OPSX (вариант 1), а flip агрегатора отложить —
...
- **generic `openspec-*`** (lifecycle-скиллы OpenSpec CLI) — де-дуплить:
  реальные копии заменять symlink-ами на OPSX. Они идентичны у всех и должны
  иметь один источник.
- **специализированные `opsx-*`** — если доменный слой встроил в них свои
  ...
  generic-базы из OPSX — отдельная целевая задача, не часть механического flip.
- **доменные скиллы** (`1c-*`, router-ы и т.п.) — всегда остаются в агрегаторе.
- **drift-gate агрегатора**: проверить, входят ли `openspec-*` в его
  source-набор. Если нет — он терпит внешние symlink-и на OPSX без правок;
  если да — научить его «generic резолвятся в OPSX» до flip.
...
- [x] Переключить их skills/commands на OPSX. Практика уточнила порядок:
  точечный перевод по одному проекту (каждый как прямой потребитель OPSX)
...
- [x] Перевести drift-gate legacy-источника. Для доменного агрегатора это
  свелось к де-дупу generic `openspec-*` в OPSX при сохранении
  специализированных `opsx-*` локально; его собственный drift-gate внешние
  symlink-и на OPSX терпит без изменений (generic skills вне его source-набора).
- [x] Решить владение OpenSpec-спеками и тестами generic skills.
- [x] Обновить локальные docs потребителей, чтобы они ссылались на OPSX как
  внешний workflow layer.
- [x] Убрать старые локальные копии, если они больше не нужны.

Результат: существующие проекты стали consumers OPSX; доменный агрегатор берёт
generic-часть из OPSX, оставляя доменный слой у себя. Практический playbook и
...
**3. Переключить wiring на OPSX** (абсолютный `/opt/opsx` как contract path).
Целевое состояние — это ровно то, что проверяет `verify-project`:

```text
.claude/skills            -> /opt/opsx/skills
.claude/commands/opsx     -> /opt/opsx/claude/commands/opsx
.codex/skills/<skill>     -> /opt/opsx/skills/<skill>   для каждого skill c SKILL.md
bin/openspec              -> /opt/opsx/bin/openspec
bin/opsx-review-verdict   -> /opt/opsx/bin/opsx-review-verdict
```

Реальные копии (`openspec-*` как каталоги) заменяются symlink-ами. Лишние
symlink-и в чужой источник, не входящие в набор OPSX-скиллов, удаляются
...
**5. Обновить `AGENTS.md`/`CLAUDE.md`** проекта: блок про OPSX должен
ссылаться на `/opt/opsx` (и на `/opt/opsx/AGENTS.shared.md`), а не на прежний
...
diff коммитится **в репозитории проекта**, не в `/opt/opsx`.

**Generic-only.** По умолчанию потребитель получает только generic OPSX-скиллы:
`.claude/skills -> /opt/opsx/skills` одним symlink-ом. Если проект реально
...
OPSX, домен -> доменный репозиторий). Заметьте: `verify-project` требует, чтобы
`.claude/skills` был **одиночным** symlink-ом на `/opt/opsx/skills`, поэтому на
