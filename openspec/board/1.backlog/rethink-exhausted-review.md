...
## Result
Реализована первая половина плана (10 из 20 задач). Работает детерминированный
разбор повторяемости, разбор исчерпания с классами и вариантами, операторские
записи выбора и изменения критерия, CLI `chrl rethink`. В точке исчерпания
сообщение несёт класс, рекомендацию и доступные варианты.

Проверено: 16 новых тестов, включая форму реального случая
`restore-accepted-task-wording` (C1/C2 повторяются, C3 закрывается,
`observed_class = repeat_defect`); 124 теста потребителей пути исчерпания;
Ruff и diff-check чисты.

Осталось: модельная роль диагностики с отдельной моделью в профиле, штатное
состояние `awaiting-operator-decision` вместо ошибки, маршрутизация выбора в
продолжение (системный ремонт, закрытие и новый план, техническое
восстановление), документация оператора, полный набор и архивация change.

## Next
- Реализовать группы 2.4, 3.1, 4 и 5 из tasks.md.
- Затем передать свежие proofs внешнему независимому review.
