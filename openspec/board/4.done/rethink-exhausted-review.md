...
либо это средовая причина — оператор получает структурированный выбор
выполнимых маршрутов, и после решения работа продолжается по той же карточке.
...
- [C4] Когда системного пути нет, доставка останавливается штатным состоянием `awaiting-operator-decision` с сохранением истории, accounting и frozen identity, и оператор получает варианты с эффектом, ценой, предусловиями и рекомендацией. Выбор фиксируется неизменяемой записью и продолжает работу по той же карточке: системный ремонт с обычным независимым ревью, либо закрытие неуспешной попытки и отдельное принятие нового плана. Маршрут технического восстановления в исчерпании не предлагается: существующее техническое восстановление принимает только исходный остановленный run до первого ревью, поэтому линия исчерпания не может его выполнить, а класс `infrastructure` только сообщает средовую причину и не даёт полномочий. Автоматического продолжения без решения оператора нет; недопустимый выбор отклоняется до запуска writer.
...
конкретное продолжение: повторное ревью, ремонт с изменённым подходом либо
закрытие попытки и новый план. Маршрута технического восстановления среди них нет
по решению оператора: существующее восстановление принимает только run до первого
ревью и линию исчерпания выполнить не может.
...
Финальная проверка после архивации: полный набор 1282 теста зелёный (5:06),
focused 115, Ruff и diff-check чистые, public-surface-scan 0 находок. Два теста
(`test_engine_snapshot` rebind и `test_self_host_e2e` publication) один раз упали
в параллельном прогоне под суммарной нагрузкой и проходят и поодиночке, и под
`-n 16` на своих файлах, и в повторном полном прогоне; это чувствительность
тяжёлых subprocess-тестов к нагрузке, а не эффект архивации.

Проверено: 41 тест диагностики (включая форму реального случая
`restore-accepted-task-wording`, кросс-проверку модельного класса и
автоматического маршрута ни у одного класса, охват линии восстановления, привязку
к плану и payload, устойчивость предпросмотра к границе секунды), 17 тестов
реального resume-маршрута учёта слотов (включая C3 —
...
## Log
- 2026-09-13T01:35:00Z третий узкий review коммита `5aaceea`: GO-WITH-NITS. Блокеры A (зависимость предпросмотра от времени) и B (недостижимая автоматическая подготовка восстановления) подтверждены закрытыми, как и C, D, E, F и finding 8; отмечено, что удаление средового маршрута честно по процессу, коду и спеке. Остались три мелочи: (1) и (2) уже исправлены более поздними коммитами `06137f3`/`28a63b8` — документация и delta-спека больше не обещают удалённый маршрут, устаревший комментарий убран; (3) у амендмента не было регрессии на границу секунды, (4) имя теста про инфраструктуру стало неточным, (5) у класса `infrastructure` не было рекомендации — все три закрыты коммитом ниже: добавлены авторизация амендмента через границу секунды, точное имя теста и рекомендация `close-attempt` (не расходует слот, попытка закрывается, оператор может принять новую).
- 2026-09-13T01:05:00Z решение оператора по открытому вопросу C4: убрать техническое восстановление из маршрутов исчерпания (выбран вариант 1 из трёх). Запись амендмента, поскольку карточка велась вне delivery-run и runtime-механизм `rethink --amend-criterion` к ней неприменим: условие `C4`, прежняя формулировка — «…либо закрытие неуспешной попытки и отдельное принятие нового плана, либо маршрут технического восстановления»; новая — те же маршруты без технического восстановления, с прямой фиксацией, что существующее техническое восстановление принимает только run до первого ревью, а класс `infrastructure` сообщает средовую причину и не даёт полномочий; автор — оператор в этой сессии; причина — маршрут недостижим для линии исчерпания, а предлагать невыполнимый маршрут нельзя. Delta-спека приведена к этой же формулировке.
