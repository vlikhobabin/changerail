...
- [x] 1.1 Add `bin/changerail-delivery-runner status` argument parsing with
  explicit path, `--run-id`, latest workspace selection, `--workspace`,
  `--runtime-root` and `--json`.
- [x] 1.2 Implement selector validation so conflicting selectors, missing
  latest records, missing files, corrupt JSON and schema-invalid
  `changerail.delivery-run.v1` inputs fail closed.
- [x] 1.3 Implement compact human-readable output for card, phase, result,
  `updated_at`, `terminal_reason` and selected status path.
- [x] 1.4 Derive canonical manifest, review verdict, review history and
...
- [x] 1.5 Ensure the command is read-only by avoiding writes to board files,
  locks, manifests, verdicts, evidence indexes, logs and status records.

## 2. Documentation

- [x] 2.1 Update `docs/changerail-contracts.md` with the single-card
  `status` command, selection modes, fail-closed validation and JSON source
  record behavior.
- [x] 2.2 Update `docs/how-it-works.md` to distinguish single-card
  `status` from aggregate `status-plan`.

## 3. Smoke Coverage

- [x] 3.1 Extend `scripts/smoke-delivery-runner.py` with synthetic fixtures for
  explicit status-path success, `--run-id` or latest selection and blocked or
  no-go diagnostics.
- [x] 3.2 Add smoke coverage for manifest `runtime_pause_reasons` rendering
  and corrupt or unsupported status failure.
- [x] 3.3 Add a read-only assertion that selected status and linked runtime
...
- [x] 4.6 Run an untracked-file trailing-whitespace scan over
