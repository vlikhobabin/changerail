...
- [x] 1.1 Preserve exactly one inline `Investigation authorization` object on
  ...
  successor ids/canonical paths, ceiling `301` and protocol flag `false`.
- [x] 1.2 Confirm the reciprocal documentation: the published investigation
  blocks authorization and future successor, authorization depends on the
  ...
  source reference. Repair only an exact successor link if current relation
  docs have drifted.
...
- [x] 2.2 Preserve independent successor acceptance at no more than 300 added
  production LOC relative to `ccccb62562e1646b595119edd3326763860f14a7`;
  treat authorization ceiling `301` only as the minimal schema-valid gate and
  retain protocol allowance `false`.
...
- [x] 3.1 Run
  `bin/openspec validate authorize-bounded-structural-public-history-scan --strict`,
