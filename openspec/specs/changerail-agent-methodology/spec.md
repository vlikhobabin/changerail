...
and required card links. Missing, stale, mismatched or over-ceiling
authorization MUST still require investigation.
...
- **WHEN** a published authorization source has an exact investigation/successor
  binding and that successor references it for up to 500 production LOC with
  the required protocol allowance
