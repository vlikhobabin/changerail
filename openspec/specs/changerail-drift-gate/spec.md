# changerail-drift-gate Specification

## Purpose
Зафиксировать workspace-level drift gate для ChangeRail consumer projects:
...
ChangeRail MUST provide `scripts/smoke-drift.py` as a workspace-level red/green gate
...
- **THEN** it classifies each discovered non-excluded project and exits `0`
  only when every non-excluded project is connected to ChangeRail source
...
#### Scenario: ChangeRail source project passes
- **WHEN** a non-excluded project passes `bin/verify-project`
- **THEN** the project class is `changerail_source`
...
- **WHEN** a non-excluded project has ChangeRail-like files but does not pass
...
- **WHEN** a non-excluded project does not expose ChangeRail-like files or valid ChangeRail
  wiring
...
- **AND** the report records `schema`, `run_id`, `changerail_root`, `config_source`,
...
ChangeRail drift gate artifacts committed to the repository MUST avoid private
