---
name: opsx-do
description: Run a supervised OPSX delivery loop for a board card or backlog file: implement each planned OpenSpec change, verify it, sync specs and archive completed changes without publishing.
---
...
Deliver the implementation part of the OPSX lifecycle:
...
$opsx-review <card-path>  # independent go/no-go review
$opsx-pub <card-path>     # scoped commit and push
```

`opsx-do` works in the foreground as the implementing session. It does not
...
Validate or normalize the manifest with `scripts/opsx_delivery_manifest.py`
...
```text
$opsx-do <card-path>
$opsx-do <card-path> --from change-slug
$opsx-do <card-path> --until change-slug
$opsx-do <card-path> --max-fix-cycles 2
```

Accept legacy prompt forms such as `/opsx:do <card-path>` and
`opsx:do <card-path>` as equivalent, but present Codex CLI instructions with
`$opsx-do`.
...
- exact next command:
  ...
  ```
  followed by `$opsx-pub <card-path>` after a fresh `go` verdict.
