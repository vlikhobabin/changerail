---
name: opsx-ff
description: Run the generic OPSX board/card fast-forward planning loop. Use when the user asks for `$opsx-ff`, `opsx:ff`, `/opsx:ff`, `opsx:fff`, or wants to take an unprocessed OpenSpec board card or story, decompose it into implementation-sized changes, update the card, and generate apply-ready OpenSpec artifacts without implementing code.
---
...
```text
$opsx-ff openspec/board/1.backlog/card.md
$opsx-ff openspec/board/2.todo/card.md
```

Useful flags when present:

```text
$opsx-ff <card> --max-changes 4
$opsx-ff <card> --from change-slug
$opsx-ff <card> --until change-slug
$opsx-ff <card> --no-move
```

Accept legacy spellings such as `/opsx:ff <card>`, `opsx:ff <card>` and
`opsx:fff <card>` as equivalent, but present Codex CLI instructions with
`$opsx-ff`.
...
3. Set `Next` to the project's delivery workflow. If the `opsx-do` surface is
   installed, use:
   ```text
   $opsx-do <card-path>
   ```
...
- delivery handoff:
  ```text
  $opsx-do <card-path>  # when the opsx-do surface is installed
  ```
