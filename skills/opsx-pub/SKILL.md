...
Finalize a delivered and independently reviewed OPSX board card:

```text
$opsx-review <card-path>  # fresh-context go/no-go verdict
$opsx-pub <card-path>     # scoped commit and push
```

Invoking `$opsx-pub` is explicit permission to create a scoped commit and push
...
```text
$opsx-pub <card-path> --no-push
$opsx-pub <card-path> --message "type(scope): summary"
$opsx-pub <card-path> --docs-only
$opsx-pub <card-path> --allow-unarchived
```

Accept legacy prompt forms such as `/opsx:pub <card>`, `opsx:pub <card>` and
`opsx:ship <card>` as equivalent, but present Codex CLI instructions with
`$opsx-pub`.
...
Read `../opsx-review/references/opsx-review-verdict.md` before publishing.
